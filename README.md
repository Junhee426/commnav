...
- **성능 분석:** 24시간 5분 간격 분석, GNSS/LEO/가상 지역항법 구성 비교, 사용자 목표 동시 충족률, 항법 시간 배분 민감도, 24시간 중 최소 가시 LEO 항법위성 수, 동시 목표를 놓친 최장 연속 구간(표본 해상도 기준 서비스 중단 추정치).
- **설정:** 500/888/1,280 km, 궤도면 1–32개 × 면당 4–32기(총 512기 이하), Walker 위상 계수 F(0–31), 경사각, 한국·UAE·동남아 관측지 및 위도·경도 직접 입력, 항법 탑재 비율, 신호 분리/시간 공유, 측정·궤도·시계 오차, 통신 링크 변수.
- **결과:** 모든 288개 표본과 실행 시점 설정을 CSV로 내보냅니다. CSV 앞부분에는 `#`으로 시작하는 메타데이터 주석(생성 시각, 모델 버전, 고정 모델 가정, 요약 통계)을 덧붙입니다. 설정을 바꾸면 이전 24시간 결과에 명시적으로 ‘이전 설정’ 표시가 나타납니다.
...
**분석 기간**과 **표본 간격**(‘통신 링크와 서비스 목표’ 고급 설정)도 직접 입력할 수 있습니다. 기본값은 기존과 같은 1,440분(24시간)·5분 간격(288개 표본)입니다. 분석 기간은 5–1,440분, 표본 간격은 1–60분이며, 분석 기간은 표본 간격으로 나누어 떨어져야 합니다. 1,440분 상한은 기준시각 하루를 전제하는 모델의 기준 회전각 가정에서 나온 것으로, 이 범위를 넘는 다일(multi-day) 분석은 지원하지 않습니다. 범위를 벗어나거나 나누어 떨어지지 않으면 오류를 표시하고 마지막 유효 설정을 유지하며, 결과 요약과 CSV 메타데이터에는 실제 사용된 표본 수·간격이 표시됩니다.
